...
class mutildepthVnet3dModuleMultiLabel(object):
    """
        A mutildepthVnet3d implementation
    ...
    def __init__(self, image_height, image_width, image_depth, channels=1, numclass=3,
                 costname=("categorical_dice",), inference=False, model_path=None):
        ...
        self.weight_loss = [0., 1., 1.]
        self.Y_pred = _create_mutildepth_conv_net(self.X, self.image_depth, self.image_width, self.image_height,
                                                  self.channels, self.phase, self.drop, self.numclass)

        self.cost = self.__get_cost(self.Y_pred, self.Y_gt, costname[0], gamma=2)
        self.Y_pred_arg = tf.reshape(tf.argmax(self.Y_pred, axis=-1),
                                     (-1, self.image_depth, self.image_height, self.image_width))
        ...
        for num in range(len(batch_xs_path)):
            ...
            subbatch_xs[num, :, :, :, :] = np.reshape(image,
                                                      (self.image_depth, self.image_height, self.image_width,
                                                       self.channels))
            subbatch_ys[num, :, :, :, :] = np.reshape(label,
                                                      (self.image_depth, self.image_height, self.image_width,
                                                       self.labelchannels))
    ...
    def train(self, train_images, train_labels, model_path, logs_path, learning_rate,
        ...
        for i in range(train_epochs):
            ...
            if i % DISPLAY_STEP == 0 or (i + 1) == train_epochs:
                ...
                save_images(gt, showwind, path=logs_path + 'gt_%d_epoch.png' % (i), pixelvalue=128)

                result = np.reshape(pred_arg[0], (self.image_depth, self.image_height, self.image_width))
                result = result.astype(np.float)
                save_images(result, showwind, path=logs_path + 'predict_%d_epoch.png' % (i), pixelvalue=128)
